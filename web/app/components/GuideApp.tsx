...
function SuggestionDialog({ area, locale, onClose }: { area: Area; locale: Locale; onClose: () => void }) {
  ...
  const completed = status === "accepted" || status === "review";
  ...
  return (
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className="modal" role="dialog" aria-modal="true" aria-labelledby="suggest-title">
        ...
        {pending && <div className="suggestion-progress" role="status" aria-live="polite"><strong>{tr(locale, "Förslaget är mottaget", "Proposal received")}</strong><span>{pendingSeconds < 45 ? tr(locale, "Kontrollerar om något behöver förtydligas…", "Checking whether anything needs clarification…") : tr(locale, "Kontrollen tar längre tid än vanligt…", "The check is taking longer than usual…")} · {pendingSeconds} s</span></div>}
