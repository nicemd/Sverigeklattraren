    ...
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className="modal" role="dialog" aria-modal="true" aria-labelledby="about-title">
        ...
        <ul>
          ...
          <li><strong>{tr(locale, "Redaktörsagenten", "The editor agent")}</strong> {tr(locale, "förvandlar användarens fritext till en precis ändring, ställer följdfrågor när underlaget inte räcker och skapar ett granskningsbart Git-förslag.", "turns a user's free text into a precise edit, asks follow-up questions when evidence is insufficient and creates a reviewable Git proposal.")}</li>
        </ul>
        ...
        <a className="source-link" href="https://github.com/nicemd/Sverigeforaren" target="_blank" rel="noreferrer"><span>{tr(locale, "Projektets Git-repository", "Project Git repository")}</span><span>↗</span></a>
      </section>
    </div>
  );
}
      ...
      {selectedRoute && <div className="route-detail-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelectedRouteId(null); }}>
        <section className="route-detail" role="dialog" aria-modal="true" aria-label={`${tr(locale, "Fältkort för", "Field card for")} ${routeName(selectedRoute)}`}>
          <div className="route-detail-head">
            <div><span className="eyebrow">{selectedRouteSector ? sectionTitle(selectedRouteSector) : tr(locale, "Sektor saknas", "Sector missing")} · {selectedRoute.number || tr(locale, "utan nummer", "no number")}</span><h2>{routeName(selectedRoute)}</h2></div>
