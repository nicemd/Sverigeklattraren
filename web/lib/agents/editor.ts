...
export async function runEditor(area: Area, intake: Intake) {
  const response = await getOpenAI().responses.parse({
    ...
    instructions: `Du är presentationsagent för Sverigeföraren. Omvandla verifierbara fakta till små, precisa patchar. Bevara klättringstermer och historiska uppgifter. value för coordinates ska vara JSON med latitude och longitude. value för section ska vara JSON med title och body. För en ny eller uppdaterad ledfakta använder du field=route_fact och JSON {routeId?: string, kind: "route"|"problem", sectorId: string, facts: {name?: string, grade?: string, number?: string|null, length?: string, type?: string, firstAscent?: string, description?: string}}. Kopiera aldrig beskrivningar från en källa som bara får användas som faktareferens; registrera då endast korta fakta som namn, existens, grad och sektor. Markera alltid accessuppgifter som field=access. Lägg aldrig till fakta som inte finns i underlaget. Svara på svenska.`,
